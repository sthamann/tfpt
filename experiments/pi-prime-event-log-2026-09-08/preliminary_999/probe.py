...
class Experiment:
 ...
 def metrics(self,a):
  ...
  for p,gaps,starts in self.ranges:
   ds=a[p-1]; counts=np.bincount(ds,minlength=10);chi=np.where(p%4==1,1.,-1.)
   ...
   out.extend([ds.mean(),np.sum((counts-len(p)/10)**2/(len(p)/10)),
    ds[p%4==1].mean()-ds[p%4==3].mean(),np.corrcoef(ds,gaps)[0,1],z.mean(),np.mean(z*z),
...
def main_tests(exp,reps):
 ...
 result['positive_control']={'planted':'digit 9 at every prime position','mean_tests_p':[float(p[0]),float(p[len(METRICS)])],'mean_observed':[float(obs[0]),float(obs[len(METRICS)])]}
...
def factoring_screen(exp):
 ...
 null=[]
 for _ in range(999):
  vals=rng.integers(0,1_000_000,(len(fixtures),32));g=np.gcd(vals,ns[:,None]);null.append(int(np.any((g>1)&(g<ns[:,None]),axis=1).sum()))
...
if __name__=='__main__':
 parser=argparse.ArgumentParser();parser.add_argument('--replicates',type=int,default=999);parser.add_argument('--reuse-data',action='store_true');args=parser.parse_args()
 t=time.perf_counter()
