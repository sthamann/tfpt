...
import gmpy2
R=Path(__file__).resolve().parent;N=13_200_032
...
def main():
    oldroot=R.parent/'prediction-gate';old=json.loads((oldroot/'data_manifest.json').read_text());out={}
    for kind in ['pi','e','sqrt2']:
        t=time.perf_counter();raw=decimals(kind,40);prefix=(oldroot/f'{kind}.digits').read_bytes()
    ...
    t=time.perf_counter();assert decimals('pi',100)==(R/'pi.digits').read_bytes()
