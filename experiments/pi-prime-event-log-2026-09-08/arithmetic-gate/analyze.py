...
assert len(rows)==len(inputs)*11 and len({(r['id'],r['method']) for r in rows})==len(rows)
...
assert hashlib.sha256((R/'engine_arithmetic.py').read_bytes()).hexdigest()==meta['script_sha256']
...
methods=['bucket_pi','bucket_e','bucket_sqrt2']+[f'bucket_random{i}' for i in range(5)]+['exact_cofactor','phase8','sequential']
summary={};test=[];rng=np.random.default_rng(202609082203)
for split in ['discovery','validation']:
 ...
 for control in ['random_mean','exact_cofactor']:
  diffs=[]
  for ident in ids:
   pi=index[ident,'bucket_pi']['smooth_count']
   other=np.mean([index[ident,f'bucket_random{j}']['smooth_count'] for j in range(5)]) if control=='random_mean' else index[ident,'exact_cofactor']['smooth_count']
...
v=summary['validation'];ratio_random=v['bucket_pi']['rank_per_amortized_second']/(sum(v[f'bucket_random{j}']['rank'] for j in range(5))/sum(v[f'bucket_random{j}']['amortized_total_seconds'] for j in range(5)))
ratio_seq=v['bucket_pi']['rank_per_amortized_second']/v['exact_cofactor']['rank_per_amortized_second']
criteria={'positive_and_holm_pass_all_four':all(t['mean_difference']>0 and t['holm_p']<.05 for t in test),'rank_throughput_2x_both':ratio_random>=2 and ratio_seq>=2,'no_success_worsening':v['bucket_pi']['factor_successes']>=v['exact_cofactor']['factor_successes'] and v['bucket_pi']['factor_successes']>=np.mean([v[f'bucket_random{j}']['factor_successes'] for j in range(5)])}
criteria={k:bool(v) for k,v in criteria.items()}
result={'verdict':'PI_RELATION_ADVANTAGE_CANDIDATE' if all(criteria.values()) else 'NO_PI_DIGIT_ADVANTAGE_AFTER_ARITHMETIC_CONDITIONING','summary':summary,'tests':test,'criteria':criteria,'validation_rank_throughput_ratio':{'vs_random':ratio_random,'vs_exact_cofactor':ratio_seq},'audit':{'instances':len(inputs),'runs':len(rows),'all_saved_success_certificates_independently_verified':cert_count,'false_factors':0,'runtime_hash_matches':True,'protocol_hash_matches':True,'answers_not_imported_by_runtime':'source-audited separately; answers read here only after run'},'scope':'Finite arithmetic-conditioned pi digit rule only. Sign-flip tests assume symmetry/exchangeability of paired differences. Timing is single-run implementation-specific; cold source costs are estimates from measured prefix generation. No regulator is recovered.'}
