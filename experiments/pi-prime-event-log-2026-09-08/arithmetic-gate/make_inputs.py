...
for split,seed in [('discovery',202609082201),('validation',202609082202)]:
...
(R/'input_manifest.json').write_text(json.dumps({'count':len(public),'distinct':len({r['n'] for r in public}),'old_overlap':len(old&{r['n'] for r in public}),'inputs_sha256':hashlib.sha256((R/'inputs.json').read_bytes()).hexdigest(),'protocol_sha256':hashlib.sha256((R/'PROTOCOL.md').read_bytes()).hexdigest(),'seeds':[202609082201,202609082202]},indent=2)+'\n')
