...
RUNNER = Path(__file__).with_name('run_round19.py').resolve()
spec = importlib.util.spec_from_file_location('round19_runner_under_test', RUNNER)
...
class RunnerFixtures(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix='round19-manifest-fixtures-')
        self.addCleanup(self.tmp.cleanup)
        self.root = Path(self.tmp.name)
        self.here = self.root/'experiments/theory-contracts'
        self.contracts = {'alpha':['check.py'], 'beta':['check.py']}
        self.inputs = ['seed/README.md', 'ROUND19_STATUS.md']
        ...
        files = {'run_round19.py':'# fixture runner provenance\n',
                 'seed/README.md':'# prerequisite proof\n',
                 'ROUND19_STATUS.md':'# bounded fixture status\n',
        ...
        for name, scripts in self.contracts.items():
            self.singles[name] = {
                'schema':'tfpt.round19.contract-validation.v1', 'status':'PASS',
        ...
        self.overall = {'schema':'tfpt.round19.research-validation.v1', 'status':'PASS',
    ...
    def test_real_coupled_checker_rejects_changed_predecessor_under_optimization(self):
        predecessor = self.here/'charge-hodge-round18/PROOF.md'
        predecessor.parent.mkdir(parents=True)
        predecessor.write_text('# deliberately changed isolated predecessor\n')
        checker = RUNNER.parent/'coupled-charge-parent-round19/checker.py'
        proc = subprocess.run([sys.executable, '-OO', '-B', str(checker),
                               '--contracts-root', str(self.here)],
                              text=True, capture_output=True, timeout=30)
        self.assertEqual(proc.returncode, 1)
        self.assertIn('AssertionError: frozen input charge-hodge-round18/PROOF.md', proc.stderr)
