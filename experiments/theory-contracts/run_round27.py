#!/usr/bin/env python3
"""Run bounded Round27 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND27_VALIDATION.json"
CONTRACTS = {
    "full-reference-control-round27": ["checker.py"],
    "reference-tail-planner-round27": ["checker.py"],
}
INPUTS = [
    "ROUND27_STATUS.md", "ROUND27_REVIEW.md", "test_round27_runner.py",
    "ROUND27_RUNNER_VALIDATION.json", "round27_algebra.py",
    "ROUND26_STATUS.md", "ROUND26_REVIEW.md", "round26_algebra.py", "run_round26.py",
    "history-sum-control-round26/PROOF.md", "cocycle-sign-obstruction-round26/PROOF.md",
    "ROUND25_STATUS.md", "ROUND25_REVIEW.md", "round25_algebra.py", "run_round25.py",
    "moving-history-memory-round25/PROOF.md", "history-determinant-solver-round25/PROOF.md",
...
PREREQUISITES = [
    "history-sum-control-round26/checker.py",
    "cocycle-sign-obstruction-round26/checker.py",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round27.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round27.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round27.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round27.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round27.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round27 validation unavailable ({type(error).__name__})"],
