#!/usr/bin/env python3
"""Run bounded Round16 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND16_VALIDATION.json"
CONTRACTS = {
...
INPUTS = [
    "ROUND16_STATUS.md", "ROUND16_REVIEW.md", "test_round16_runner.py",
    "ROUND16_RUNNER_VALIDATION.json", "ROUND15_STATUS.md", "ROUND15_REVIEW.md",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round16.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round16.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round16.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round16.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round16.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round16 validation unavailable ({type(error).__name__})"],
