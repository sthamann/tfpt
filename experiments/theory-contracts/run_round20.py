#!/usr/bin/env python3
"""Run bounded Round20 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND20_VALIDATION.json"
CONTRACTS = {
...
INPUTS = [
    "ROUND20_STATUS.md", "ROUND20_REVIEW.md", "test_round20_runner.py",
    "ROUND20_RUNNER_VALIDATION.json",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round20.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round20.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round20.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round20.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round20.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round20 validation unavailable ({type(error).__name__})"],
