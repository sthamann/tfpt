#!/usr/bin/env python3
"""Run bounded Round21 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND21_VALIDATION.json"
CONTRACTS = {
    "dressed-hopping-round21": ["checker.py"],
    "charge-momentum-round21": ["checker.py"],
}
INPUTS = [
    "ROUND21_STATUS.md", "ROUND21_REVIEW.md", "test_round21_runner.py",
    "ROUND21_RUNNER_VALIDATION.json", "round21_algebra.py",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round21.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round21.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round21.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round21.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round21.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round21 validation unavailable ({type(error).__name__})"],
