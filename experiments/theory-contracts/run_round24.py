#!/usr/bin/env python3
"""Run bounded Round24 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND24_VALIDATION.json"
CONTRACTS = {
    "finite-density-background-round24": ["checker.py"],
    "local-gaussian-elimination-round24": ["checker.py"],
}
INPUTS = [
    "ROUND24_STATUS.md", "ROUND24_REVIEW.md", "test_round24_runner.py",
    "ROUND24_RUNNER_VALIDATION.json", "round24_algebra.py",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round24.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round24.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round24.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round24.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round24.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round24 validation unavailable ({type(error).__name__})"],
