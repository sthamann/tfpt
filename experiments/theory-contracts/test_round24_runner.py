...
RUNNER = Path(__file__).with_name('run_round24.py').resolve()
spec = importlib.util.spec_from_file_location('round24_runner_under_test', RUNNER)
...
class RunnerFixtures(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix='round24-manifest-fixtures-')
        self.addCleanup(self.tmp.cleanup)
        self.root = Path(self.tmp.name)
        self.here = self.root/'experiments/theory-contracts'
        self.contracts = {'alpha':['check.py'], 'beta':['check.py']}
        self.inputs = ['seed/README.md', 'ROUND24_STATUS.md']
        ...
        files = {'run_round24.py':'# fixture runner provenance\n',
                 'seed/README.md':'# prerequisite proof\n',
                 'ROUND24_STATUS.md':'# bounded fixture status\n',
        ...
        for name, scripts in self.contracts.items():
            self.singles[name] = {
                'schema':'tfpt.round24.contract-validation.v1', 'status':'PASS',
        ...
        self.overall = {'schema':'tfpt.round24.research-validation.v1', 'status':'PASS',
    ...
    def pinned_fixture(self):
        spec = importlib.util.spec_from_file_location('r24_pins', RUNNER.parent/'round24_algebra.py')
    ...
    def consumers_reject(self, name):
        for folder in ('finite-density-background-round24', 'local-gaussian-elimination-round24'):
    ...
    def checker_mutant(self, folder, old, new, expected_error, mutate_common=False):
        source_path = RUNNER.parent/('round24_algebra.py' if mutate_common else folder+'/checker.py')
        source = source_path.read_text()
        self.assertEqual(source.count(old), 1)
        checker = self.here/folder/'checker.py'
        checker.parent.mkdir(parents=True, exist_ok=True)
        checker.write_bytes((RUNNER.parent/folder/'checker.py').read_bytes())
        (self.here/'round24_algebra.py').write_bytes((RUNNER.parent/'round24_algebra.py').read_bytes())
        target = self.here/'round24_algebra.py' if mutate_common else checker
    ...
    def test_finite_density_mass_scaling_is_not_diluted_again(self):
        self.checker_mutant('finite-density-background-round24',
            'return mass2+2*coupling*background_energy',
            'return mass2+2*coupling*background_energy/count',
            'declared lambda equals N times u gives a finite mass shift', True)

    def test_hard_gaussian_fluctuations_cannot_be_dropped(self):
        self.checker_mutant('finite-density-background-round24',
            'return w+2*z*c+c**2+4*z*v+2*v**2',
            'return w+2*z*c+c**2+4*z*v',
            'hard Gaussian contractions and zero-mode fourth moment are all retained', True)

    def test_neutral_energy_mean_cannot_be_set_to_zero(self):
        self.checker_mutant('finite-density-background-round24',
            'mean=2*eps*(2*eta+eta**2)', 'mean=s.Integer(0)',
            'second-order neutral energy mean is not dropped', True)

    def test_gaussian_determinant_cannot_be_dropped(self):
        self.checker_mutant('local-gaussian-elimination-round24',
            'return 1/s.sqrt(determinant)', 'return s.Integer(1)',
            'Gaussian integral keeps its determinant normalization', True)

    def test_deleted_boundary_edges_cannot_fake_dirichlet_block(self):
        self.checker_mutant('local-gaussian-elimination-round24',
            'result[i,i]=mass2+6+potential.get(x,0)',
            'result[i,i]=mass2+3+potential.get(x,0)',
            'fast block keeps the physical six-neighbor diagonal', True)

    def test_induced_kinetic_term_cannot_be_dropped(self):
        self.checker_mutant('local-gaussian-elimination-round24',
            'Z=s.eye(A.rows)+C*inverse**2*C.T', 'Z=s.eye(A.rows)',
            'induced kinetic matrix is retained, not silently set to identity', True)
