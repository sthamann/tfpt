#!/usr/bin/env python3
"""Run bounded Round13 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND13_VALIDATION.json"
CONTRACTS = {
    "clock-shell-round13": ["clock_shell_checker.py"],
    "weak-constraints-round13": ["weak_constraints_check.py", "redteam_weak_selection.py"],
    "weak-local-construction-round13": ["local_weak_check.py"],
    "covariant-domain-round13": ["covariant_domain_checker.py"],
}
INPUTS = [
    "ROUND13_STATUS.md", "test_round13_runner.py", "ROUND13_RUNNER_VALIDATION.json",
    "free-scalar-3d/README.md", "constraint-dressing/README.md",
    "mixed-constraints-round11/README.md",
    "local-positive-auxiliary/README.md", "local-positive-auxiliary/SECOND_CLASS.md",
    "local-positive-auxiliary/QUANTUM_DOMAIN.md",
    "homogeneous-completion-round11/QUANTUM_CLOCK.md",
    "reduced-locality-round11/PROOF.md", "momentum-deformation-round12/PROOF.md",
    "unreduced-domain-round12/README.md", "relational-observables-round12/README.md",
    "canonical-shear-round12/PROOF.md",
]
PREREQUISITES = [
    "free-scalar-3d/free_scalar_ward.py",
    "momentum-deformation-round12/momentum_deformation_check.py",
    "momentum-deformation-round12/redteam_momentum_check.py",
    "unreduced-domain-round12/unreduced_domain_checker.py",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round13.py", ROOT/"verification/v1035_matter_coupling.py"]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round13.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round13.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round13.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round13.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round13 validation unavailable ({type(error).__name__})"],
