#!/usr/bin/env python3
"""Run bounded Round15 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND15_VALIDATION.json"
CONTRACTS = {
    "minimal-stability-round15": ["minimal_stability_check.py"],
...
INPUTS = [
    "ROUND15_STATUS.md", "ROUND15_REVIEW.md", "test_round15_runner.py",
    "ROUND15_RUNNER_VALIDATION.json", "ROUND14_STATUS.md",
    "free-scalar-3d/README.md", "constraint-dressing/README.md",
    "mixed-constraints-round11/README.md", "local-positive-auxiliary/README.md",
    "local-positive-auxiliary/QUANTUM_DOMAIN.md",
    "firstclass-completion-round14/PROOF.md",
    "joint-constraint-model-round14/PROOF.md",
    "reduced-locality-round11/README.md", "relational-observables-round12/README.md",
    "escape-round14/PROOF.md", "escape-round14/escape_check.py",
]
ROOT_INPUTS = [
...
PREREQUISITES = [
    ...
    "escape-round14/redteam_full_source_check.py",
]
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round15.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round15.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round15.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round15.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round15.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round15 validation unavailable ({type(error).__name__})"],
