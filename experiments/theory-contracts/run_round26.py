#!/usr/bin/env python3
"""Run bounded Round26 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND26_VALIDATION.json"
CONTRACTS = {
    "history-sum-control-round26": ["checker.py"],
    "cocycle-sign-obstruction-round26": ["checker.py"],
}
INPUTS = [
    "ROUND26_STATUS.md", "ROUND26_REVIEW.md", "test_round26_runner.py",
    "ROUND26_RUNNER_VALIDATION.json", "round26_algebra.py",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round26.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round26.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round26.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round26.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round26.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round26 validation unavailable ({type(error).__name__})"],
