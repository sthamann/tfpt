#!/usr/bin/env python3
"""Run bounded Round23 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND23_VALIDATION.json"
CONTRACTS = {
    "translation-locality-round23": ["checker.py"],
    "soft-sector-round23": ["checker.py"],
}
INPUTS = [
    "ROUND23_STATUS.md", "ROUND23_REVIEW.md", "test_round23_runner.py",
    "ROUND23_RUNNER_VALIDATION.json", "round23_algebra.py",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round23.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round23.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round23.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round23.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round23.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round23 validation unavailable ({type(error).__name__})"],
