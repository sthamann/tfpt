...
RUNNER = Path(__file__).with_name('run_round23.py').resolve()
spec = importlib.util.spec_from_file_location('round23_runner_under_test', RUNNER)
...
class RunnerFixtures(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix='round23-manifest-fixtures-')
        self.addCleanup(self.tmp.cleanup)
        self.root = Path(self.tmp.name)
        self.here = self.root/'experiments/theory-contracts'
        self.contracts = {'alpha':['check.py'], 'beta':['check.py']}
        self.inputs = ['seed/README.md', 'ROUND23_STATUS.md']
        ...
        files = {'run_round23.py':'# fixture runner provenance\n',
                 'seed/README.md':'# prerequisite proof\n',
                 'ROUND23_STATUS.md':'# bounded fixture status\n',
        ...
        for name, scripts in self.contracts.items():
            self.singles[name] = {
                'schema':'tfpt.round23.contract-validation.v1', 'status':'PASS',
        ...
        self.overall = {'schema':'tfpt.round23.research-validation.v1', 'status':'PASS',
    ...
    def pinned_fixture(self):
        spec = importlib.util.spec_from_file_location('r23_pins', RUNNER.parent/'round23_algebra.py')
    ...
    def consumers_reject(self, name):
        for folder in ('translation-locality-round23', 'soft-sector-round23'):
    ...
    def checker_mutant(self, folder, old, new, expected_error, mutate_common=False):
        source_path = RUNNER.parent/('round23_algebra.py' if mutate_common else folder+'/checker.py')
        source = source_path.read_text()
        self.assertEqual(source.count(old), 1)
        checker = self.here/folder/'checker.py'
        checker.parent.mkdir(parents=True, exist_ok=True)
        checker.write_bytes((RUNNER.parent/folder/'checker.py').read_bytes())
        (self.here/'round23_algebra.py').write_bytes((RUNNER.parent/'round23_algebra.py').read_bytes())
        target = self.here/'round23_algebra.py' if mutate_common else checker
    ...
    def test_wrapped_selection_cannot_fake_locality(self):
        self.checker_mutant('translation-locality-round23',
            'return wrap(k+delta, size)-k == delta',
            'return (wrap(k+delta, size)-k) % size == delta % size',
            'L5: averaged source has the nonzero removed-plane kernel', True)

    def test_alias_boundary_is_not_safe(self):
        self.checker_mutant('soft-sector-round23',
            'return 4*cutoff < size', 'return 3*cutoff < size',
            'boundary equality is not accepted as an alias-free theorem', True)

    def test_identical_boson_normalization_is_required(self):
        self.checker_mutant('soft-sector-round23',
            'return s.sqrt(2)/(2*count*omega)', 'return 1/(2*count*omega)',
            'identical hard pair retains its exact Bose normalization', True)

    def test_missing_duhamel_term_is_rejected(self):
        self.checker_mutant('soft-sector-round23',
            'return 4*coupling*time_abs*s.sqrt', 'return 2*coupling*time_abs*s.sqrt',
            'all-time comparison retains the M+2 ladder output and both Duhamel terms', True)

    def test_wrong_reference_vacuum_is_rejected(self):
        self.checker_mutant('soft-sector-round23',
            'return physical_omega', 'return s.Integer(1)',
            'soft scalar reference annihilates the physical free vacuum', True)
