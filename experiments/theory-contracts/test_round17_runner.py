...
RUNNER = Path(__file__).with_name('run_round17.py').resolve()
spec = importlib.util.spec_from_file_location('round17_runner_under_test', RUNNER)
...
class RunnerFixtures(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix='round17-manifest-fixtures-')
        self.addCleanup(self.tmp.cleanup)
        self.root = Path(self.tmp.name)
        self.here = self.root/'experiments/theory-contracts'
        self.contracts = {'alpha':['check.py'], 'beta':['check.py']}
        self.inputs = ['seed/README.md', 'ROUND17_STATUS.md']
        ...
        files = {'run_round17.py':'# fixture runner provenance\n',
                 'seed/README.md':'# prerequisite proof\n',
                 'ROUND17_STATUS.md':'# bounded fixture status\n',
        ...
        for name, scripts in self.contracts.items():
            self.singles[name] = {
                'schema':'tfpt.round17.contract-validation.v1', 'status':'PASS',
        ...
        self.overall = {'schema':'tfpt.round17.research-validation.v1', 'status':'PASS',
