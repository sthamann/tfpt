...
RUNNER = Path(__file__).with_name('run_round15.py').resolve()
spec = importlib.util.spec_from_file_location('round15_runner_under_test', RUNNER)
...
class RunnerFixtures(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix='round15-manifest-fixtures-')
        self.addCleanup(self.tmp.cleanup)
        self.root = Path(self.tmp.name)
        self.here = self.root/'experiments/theory-contracts'
        self.contracts = {'alpha':['check.py'], 'beta':['check.py']}
        self.inputs = ['seed/README.md', 'ROUND15_STATUS.md']
        ...
        files = {'run_round15.py':'# fixture runner provenance\n',
                 'seed/README.md':'# prerequisite proof\n',
                 'ROUND15_STATUS.md':'# bounded fixture status\n',
        ...
        for name, scripts in self.contracts.items():
            self.singles[name] = {
                'schema':'tfpt.round15.contract-validation.v1', 'status':'PASS',
        ...
        self.overall = {'schema':'tfpt.round15.research-validation.v1', 'status':'PASS',
