#!/usr/bin/env python3
"""Run bounded Round19 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND19_VALIDATION.json"
CONTRACTS = {
...
INPUTS = [
    "ROUND19_STATUS.md", "ROUND19_REVIEW.md", "test_round19_runner.py",
    "ROUND19_RUNNER_VALIDATION.json",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round19.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round19.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round19.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round19.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round19.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round19 validation unavailable ({type(error).__name__})"],
