...
RUNNER = Path(__file__).with_name('run_round18.py').resolve()
spec = importlib.util.spec_from_file_location('round18_runner_under_test', RUNNER)
...
class RunnerFixtures(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix='round18-manifest-fixtures-')
        self.addCleanup(self.tmp.cleanup)
        self.root = Path(self.tmp.name)
        self.here = self.root/'experiments/theory-contracts'
        self.contracts = {'alpha':['check.py'], 'beta':['check.py']}
        self.inputs = ['seed/README.md', 'ROUND18_STATUS.md']
        ...
        files = {'run_round18.py':'# fixture runner provenance\n',
                 'seed/README.md':'# prerequisite proof\n',
                 'ROUND18_STATUS.md':'# bounded fixture status\n',
        ...
        for name, scripts in self.contracts.items():
            self.singles[name] = {
                'schema':'tfpt.round18.contract-validation.v1', 'status':'PASS',
        ...
        self.overall = {'schema':'tfpt.round18.research-validation.v1', 'status':'PASS',
