...
RUNNER = Path(__file__).with_name('run_round22.py').resolve()
spec = importlib.util.spec_from_file_location('round22_runner_under_test', RUNNER)
...
class RunnerFixtures(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix='round22-manifest-fixtures-')
        self.addCleanup(self.tmp.cleanup)
        self.root = Path(self.tmp.name)
        self.here = self.root/'experiments/theory-contracts'
        self.contracts = {'alpha':['check.py'], 'beta':['check.py']}
        self.inputs = ['seed/README.md', 'ROUND22_STATUS.md']
        ...
        files = {'run_round22.py':'# fixture runner provenance\n',
                 'seed/README.md':'# prerequisite proof\n',
                 'ROUND22_STATUS.md':'# bounded fixture status\n',
        ...
        for name, scripts in self.contracts.items():
            self.singles[name] = {
                'schema':'tfpt.round22.contract-validation.v1', 'status':'PASS',
        ...
        self.overall = {'schema':'tfpt.round22.research-validation.v1', 'status':'PASS',
    ...
    def pinned_fixture(self):
        spec = importlib.util.spec_from_file_location('r21_pins', RUNNER.parent/'round22_algebra.py')
    ...
    def consumers_reject(self, name):
        for folder in ('positive-hopping-band-round22', 'positive-hopping-momentum-round22', 'translation-completion-round22'):
    ...
    def checker_mutant(self, folder, old, new, expected_error, mutate_common=False):
        source_path = RUNNER.parent/('round22_algebra.py' if mutate_common else folder+'/checker.py')
        source = source_path.read_text()
        self.assertEqual(source.count(old), 1)
        checker = self.here/folder/'checker.py'
        checker.parent.mkdir(parents=True, exist_ok=True)
        checker.write_bytes((RUNNER.parent/folder/'checker.py').read_bytes())
        (self.here/'round22_algebra.py').write_bytes((RUNNER.parent/'round22_algebra.py').read_bytes())
        target = self.here/'round22_algebra.py' if mutate_common else checker
    ...
    def test_undersampled_positive_average_is_rejected(self):
        self.checker_mutant('translation-completion-round22',
            'AVERAGING_GRID = 7', 'AVERAGING_GRID = 3',
            'quadrature cannot alias any square-factor frequency difference', True)

    def test_unbounded_source_graph_cost_cannot_be_omitted(self):
        self.checker_mutant('positive-hopping-band-round22',
            'error = dvec*(2+2*eps)', 'error = dvec*2',
            'unbounded-source graph error is included', True)

    def test_truncated_charge_hopping_cannot_fake_a_band(self):
        self.checker_mutant('positive-hopping-band-round22',
            'out = {n:a for n,a in out.items() if a != 0}',
            'out = {n:a for n,a in out.items() if a != 0 and n in basis_index}',
            'one-charge subspace really leaks under original hopping')

    def test_reversed_scalar_momentum_sign_is_rejected(self):
        self.checker_mutant('positive-hopping-momentum-round22',
            'return s.sin(2*s.pi*k[0]/3)', 'return -s.sin(2*s.pi*k[0]/3)',
            'two resonances demand distinct real charge momentum differences', True)
