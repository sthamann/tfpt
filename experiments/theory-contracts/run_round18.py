#!/usr/bin/env python3
"""Run bounded Round18 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND18_VALIDATION.json"
CONTRACTS = {
    "local-source-stability-round18": ["checker.py"],
    "preconditioned-local-parent-round18": ["checker.py"],
    "auxiliary-source-obstruction-round18": ["checker.py"],
    "charge-hodge-round18": ["checker.py"],
}
INPUTS = [
    "ROUND18_STATUS.md", "ROUND18_REVIEW.md", "test_round18_runner.py",
    "ROUND18_RUNNER_VALIDATION.json",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round18.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round18.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round18.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round18.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round18.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round18 validation unavailable ({type(error).__name__})"],
