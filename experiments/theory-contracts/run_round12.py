#!/usr/bin/env python3
"""Run bounded Round12 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND12_VALIDATION.json"
CONTRACTS = {
    "relational-observables-round12": ["reference_frame_checker.py"],
    "canonical-shear-round12": ["shear_check.py"],
    "momentum-deformation-round12": ["momentum_deformation_check.py", "redteam_momentum_check.py"],
    "unreduced-domain-round12": ["unreduced_domain_checker.py"],
}
INPUTS = [
    "ROUND12_STATUS.md", "test_round12_runner.py", "ROUND12_RUNNER_VALIDATION.json",
    "constraint-dressing/README.md", "free-scalar-3d/README.md",
    "free-scalar-3d/free_scalar_ward.py", "mixed-constraints-round11/README.md",
    ...
    "reduced-locality-round11/PROOF.md", "reduced-locality-round11/locality_check.py",
    "reduced-locality-round11/redteam_source_projector.py",
]
PREREQUISITES = [
    ...
    "homogeneous-completion-round11/quantum_clock.py",
    "reduced-locality-round11/locality_check.py",
    "reduced-locality-round11/redteam_source_projector.py",
]
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round12.py", ROOT/"verification/v1035_matter_coupling.py"]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round12.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round12.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round12.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round12.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round12 validation unavailable ({type(error).__name__})"],
