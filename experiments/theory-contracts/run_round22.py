#!/usr/bin/env python3
"""Run bounded Round22 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND22_VALIDATION.json"
CONTRACTS = {
...
INPUTS = [
    "ROUND22_STATUS.md", "ROUND22_REVIEW.md", "test_round22_runner.py",
    "ROUND22_RUNNER_VALIDATION.json", "round22_algebra.py",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round22.py"] + [ROOT/p for p in ROOT_INPUTS]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round22.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round22.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round22.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round22.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round22 validation unavailable ({type(error).__name__})"],
