#!/usr/bin/env python3
"""Run bounded Round14 non-RH checks; emit JSON without writing files.
...
ROOT = HERE.parents[1]
OVERALL_NAME = "ROUND14_VALIDATION.json"
CONTRACTS = {
    "auxiliary-domain-round14": ["auxiliary_domain_check.py"],
    "common-domain-round14": ["common_domain_checker.py"],
    "escape-round14": ["escape_check.py", "redteam_full_source_check.py"],
    "galerkin-control-round14": ["galerkin_control_check.py"],
    "firstclass-completion-round14": ["firstclass_completion_check.py"],
    "joint-constraint-model-round14": ["joint_constraint_check.py"],
}
INPUTS = [
    "ROUND14_STATUS.md", "test_round14_runner.py", "ROUND14_RUNNER_VALIDATION.json",
    ...
    "unreduced-domain-round12/README.md",
    "covariant-domain-round13/README.md", "clock-shell-round13/README.md",
...
def audited_paths() -> list[Path]:
    ...
    paths += [HERE/"run_round14.py", ROOT/"verification/v1035_matter_coupling.py"]
...
def check_manifests() -> int:
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        if manifest.get("schema") != "tfpt.round14.contract-validation.v1":
    ...
    if overall is not None:
        if overall.get("schema") != "tfpt.round14.research-validation.v1":
...
def execute_checks() -> int:
    ...
    result = {"schema": "tfpt.round14.research-validation.v1", "contracts": {}}
    ...
    for contract, scripts in CONTRACTS.items():
        ...
        result["contracts"][contract] = {
            "schema": "tfpt.round14.contract-validation.v1",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "FAIL", "errors": [f"Round14 validation unavailable ({type(error).__name__})"],
