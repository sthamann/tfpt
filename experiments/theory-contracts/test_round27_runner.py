...
RUNNER = Path(__file__).with_name('run_round27.py').resolve()
spec = importlib.util.spec_from_file_location('round27_runner_under_test', RUNNER)
...
class RunnerFixtures(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix='round27-manifest-fixtures-')
        self.addCleanup(self.tmp.cleanup)
        self.root = Path(self.tmp.name)
        self.here = self.root/'experiments/theory-contracts'
        self.contracts = {'alpha':['check.py'], 'beta':['check.py']}
        self.inputs = ['seed/README.md', 'ROUND27_STATUS.md']
        ...
        files = {'run_round27.py':'# fixture runner provenance\n',
                 'seed/README.md':'# prerequisite proof\n',
                 'ROUND27_STATUS.md':'# bounded fixture status\n',
        ...
        for name, scripts in self.contracts.items():
            self.singles[name] = {
                'schema':'tfpt.round27.contract-validation.v1', 'status':'PASS',
        ...
        self.overall = {'schema':'tfpt.round27.research-validation.v1', 'status':'PASS',
    ...
    def pinned_fixture(self):
        spec = importlib.util.spec_from_file_location('r27_pins', RUNNER.parent/'round27_algebra.py')
    ...
    def consumers_reject(self, name):
        for folder in ('full-reference-control-round27', 'reference-tail-planner-round27'):
    ...
    def checker_mutant(self, folder, old, new, expected_error, mutate_common=False):
        source_path = RUNNER.parent/('round27_algebra.py' if mutate_common else folder+'/checker.py')
        source = source_path.read_text()
        self.assertEqual(source.count(old), 1)
        checker = self.here/folder/'checker.py'
        checker.parent.mkdir(parents=True, exist_ok=True)
        checker.write_bytes((RUNNER.parent/folder/'checker.py').read_bytes())
        (self.here/'round27_algebra.py').write_bytes((RUNNER.parent/'round27_algebra.py').read_bytes())
        target = self.here/'round27_algebra.py' if mutate_common else checker
    ...
    def test_root_rounding_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "return lo\n",
            "return max(0,lo-1)\n",
            "integer root rounds upwards even immediately after perfect powers", True)

    def test_charge_coordinates_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "return (1+2*k)**(8*count)",
            "return (1+2*k)**count",
            "reference Gaussian envelope retains the original eight coordinates", True)

    def test_reference_heat_split_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "theta_upper(beta/2,count)",
            "theta_upper(beta,count)",
            "charge tail uses full-reference half beta and a Tth root", True)

    def test_reference_root_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "root=ceil_root(theta_upper(beta/2,count),slices)",
            "root=theta_upper(beta/2,count)",
            "charge tail uses full-reference half beta and a Tth root", True)

    def test_hop_factorial_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "return theta**(order+1)/math.factorial(order+1)/(1-theta/(order+2))",
            "return theta**(order+1)/(1-theta/(order+2))",
            "relative word bound has factorial and correct geometric ratio", True)

    def test_hop_ratio_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "return theta**(order+1)/math.factorial(order+1)/(1-theta/(order+2))",
            "return theta**(order+1)/math.factorial(order+1)/(1-theta/(order+1))",
            "relative word bound has factorial and correct geometric ratio", True)

    def test_memory_sign_budget_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "return 3**math.ceil(theta)*B/(1-B)",
            "return B/(1-B)",
            "memory budget retains exponent margin and cancellation factor", True)

    def test_memory_relative_margin_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "return 3**math.ceil(theta)*B/(1-B)",
            "return 3**math.ceil(theta)*B",
            "zero hopping has no word tail or artificial sign factor", True)

    def test_scalar_coupling_square_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "epsilon=delta**2*36*tail",
            "epsilon=delta**2*6*tail",
            "memory tail keeps wrap factor both temporal directions and scalar coupling", True)

    def test_determinant_half_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "B=epsilon*retained*slices*gm/(2*(1-eta))",
            "B=epsilon*retained*slices*gm/(1-eta)",
            "memory logarithm retains half determinant trace and positive margin", True)

    def test_all_block_pinching_mutant_is_rejected(self):
        self.checker_mutant("full-reference-control-round27",
            "i//block_size==j//block_size else 0",
            "i//block_size==j//block_size==0 else 0",
            "pinching retains every charge block and its full scalar space", True)

    def test_word_shift_mutant_is_rejected(self):
        self.checker_mutant("full-reference-control-round27",
            "result=result*U*(projector",
            "result=result*(projector",
            "dropping the charge-tail Tth root has an explicit counterexample", True)

    def test_false_partition_execution_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "'partition_sum_executed':False",
            "'partition_sum_executed':True",
            "original comparison budget certifies without full partition execution", True)

    def test_false_local_determinant_mutant_is_rejected(self):
        self.checker_mutant("reference-tail-planner-round27",
            "'exact_fast_determinant_retained':True",
            "'exact_fast_determinant_retained':False",
            "combined plan keeps full determinant and never invents an evaluated sum", True)
